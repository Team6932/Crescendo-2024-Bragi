...
/*
 * THIS IS FULLY COPY PASTED FROM THE YAGSL EXAMPLE CODE.
 * It worked, so I didn't complain. 
 * I might add more comments later. 
 */
...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    SwerveDriveTelemetry.verbosity = TelemetryVerbosity.MACHINE; //NONE, LOW, HIGH, or MACHINE
    try {
...
//////// WPILib System Identification Tool (no clue how to use it) /////////////
  ...
  public Command doubleDriveCommand(double X, double Y, double rot) {
    return run(() -> {
      swerveDrive.drive(new Translation2d(Math.pow(X, 3) * swerveDrive.getMaximumVelocity(), 
        Math.pow(Y, 3) * swerveDrive.getMaximumVelocity()),
